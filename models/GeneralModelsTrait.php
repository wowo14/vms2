...
trait GeneralModelsTrait
{
    ...
    public function range($d, $type)
    { //filter condition date between
    ...
    public static function hashid($params)
    { // id as primarykey
        $class = self::class;
    ...
    public static function optionsSettingtype($type, $callback)
    { //example $model::optionsSettingtype(string 'type', string|array|function);
    ...
    // {
    //     $dep = self::settagdep('tag_petugas');
    //     $peg = collect(Pegawai::find()->cache(24 * 60 * 60, $dep)->all());
    //     // if(self::getModelname()){
    //     $peg = $peg->where('hak_akses', 'staff'); // add filter sesuai petugas dan hak akses
    //     // }
    //     $peg = $peg->pluck('nama', 'id')->toArray();
    //     return $peg;
    // }
    // public static function formassignpetugas($pks = null)
    // {
    //     $optpetugas = self::getAllpetugas();
    //     $msg = '<form action="/notaris/assign" method="post">';
    //     if ($pks) {
    //         $msg .= '<div><input type="hidden" name="pks" value="' . $pks . '"/>';
    //     }
    //     $msg .= '<div><label for="selectOptions">Pilih Petugas:</label>
    //             <select id="selectOptions" name="petugas_id">
    //             <option value="">...</option>';
    ...
    public function getIncompanygrouporadmin()
    { // boolean
        if ($this->getIsVendor()) {
    ...
    public function getIsBase64Encoded($str)
    { //boolean
